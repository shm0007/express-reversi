...
const HOST = "192.168.1.4"; // Hosting Server change when you make it live on server according to your hosting server
...
io.on("connection", (client) => {
  ...
  client.on("selectOpponent", (data) => {
    ...
    if (!sockets[data.id].is_playing) {
      ...
      players[sockets[data.id].mobile_number].played =
        players[sockets[data.id].mobile_number].played + 1;
      players[sockets[client.id].mobile_number].played =
        players[sockets[client.id].mobile_number].played + 1;
  ...
  client.on("selectCell", (data) => {
    games[data.gameId].playboard[data.i][data.j] =
      games[data.gameId][games[data.gameId].whose_turn].sign;
    ...
    if (isDraw) games[data.gameId].game_status = "draw";

    for (let i = 0; i < winCombinations.length; i++) {
      var tempComb =
        games[data.gameId].playboard[winCombinations[i][0][0]][
          winCombinations[i][0][1]
        ] +
        games[data.gameId].playboard[winCombinations[i][1][0]][
          winCombinations[i][1][1]
        ] +
        games[data.gameId].playboard[winCombinations[i][2][0]][
      ...
      if (tempComb === "xxx" || tempComb === "ooo") {
        ...
        players[
          games[data.gameId][games[data.gameId].game_winner].mobile_number
        ].won++;
      }
    }
    if (games[data.gameId].game_status == "draw") {
      players[games[data.gameId][games[data.gameId].player1].mobile_number]
        .draw++;
      players[games[data.gameId][games[data.gameId].player2].mobile_number]
        .draw++;
    }
    games[data.gameId].whose_turn =
      games[data.gameId].whose_turn == games[data.gameId].player1
    ...
    if (
      ...
      gameBetweenInterval = setInterval(() => {
        ...
        if (gameBetweenSeconds == 0) {
          ...
          players[sockets[games[data.gameId].player1].mobile_number].played =
            players[sockets[games[data.gameId].player1].mobile_number].played +
            1;
          players[sockets[games[data.gameId].player2].mobile_number].played =
            players[sockets[games[data.gameId].player2].mobile_number].played +
            1;
          ...
          games[gameId][games[data.gameId].player1] = {
            ...
            draw:
              players[sockets[games[data.gameId].player1].mobile_number].draw,
          };
          games[gameId][games[data.gameId].player2] = {
            ...
            draw:
              players[sockets[games[data.gameId].player2].mobile_number].draw,
          };
  ...
  client.on("disconnect", () => {
    ...
    if (typeof sockets[client.id] != "undefined") {
      if (sockets[client.id].is_playing) {
        ...
        players[
          sockets[games[sockets[client.id].game_id].player1].mobile_number
        ].played--;
        players[
          sockets[games[sockets[client.id].game_id].player2].mobile_number
        ].played--;
